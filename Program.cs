    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            {//выражения принимают значения true
                ...
                Console.WriteLine("Sa ostsid istme :) "); //Выводит сообщение о покупке места
                Console.WriteLine("Kas soovite osta teise istme?");
                Console.WriteLine(" 1 - jah 0 - ei");
                int final_vibor = int.Parse(Console.ReadLine());
                if (final_vibor == 1)//Если пользователь желает продолжить покупку билетов то при нажатии 1 цикл покупки повторяется
                {
                    Console.Clear();
                    continue;
                }
                else//Программа завершается
                {
                    break;
                }
            }
